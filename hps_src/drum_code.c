...
#define drum_size 18
#define drum_middle 9
int copy_size = drum_size*drum_size*4 ;

//non-linear parameters
#define rho_initial float2fix28(0.25)
#define g_tension float2fix28(0.07)
...
#define middle_node (&drum_n[drum_middle][drum_middle])
#define nonlinear_rho(a) multfix28(a, min(rho_initial + multfix28(multfix28(g_tension, *middle_node), multfix28(g_tension, *middle_node)), float2fix28(0.49)))
		...
		while (((*audio_fifo_data_ptr>>24)& 0xff) > 1) {
			...
			for (i=1; i<drum_size-1; i++){
				for (j=1; j<drum_size-1; j++){
					new_drum_temp = nonlinear_rho(drum_n[i-1][j] + drum_n[i+1][j] + drum_n[i][j-1] + drum_n[i][j+1] - times4pt0(drum_n[i][j]));
